...
> Except for the Data Filtering demo (see its description below), these repositories demonstrate how to integrate the Code Effects Rule Editor into web applications, persist user-created business rules, and execute them on the server. Each demo targets a different UI framework or source object model while following the same end-to-end workflow.
